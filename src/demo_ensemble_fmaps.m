...
%% Ensembling.    
    ...
    for i=1:num_of_maps        
        ...
        error_i   = sum(sum(abs((fmap.fmap * Fs) - Ft), 1)) ./ feats_i;      % Average alignment error of probe functions.
        
        Fs = fmap.source_basis.project_functions(fmap.source_neigs, feats1.F);
        Fs = divide_columns(Fs, sqrt(sum(Fs.^2)));                           % Normalize features.
        Ft = fmap.target_basis.project_functions(fmap.target_neigs, feats2.F);               
        Ft = divide_columns(Ft, sqrt(sum(Ft.^2)));                
        
        a_error_i   = sum(sum(abs((fmap.fmap * Fs) - Ft), 1)) ./ all_feats;  % Average alignment error of all functions.

        ensembles{i}.map         = fmap.fmap;              
        ensembles{i}.error       = error_i;
        ensembles{i}.error_all   = a_error_i;
...
%   3. Combine the learned errors to create a map that emphasizes them.
    ...
    for i=1:num_of_maps 
        wi = (1 - (ensembles{i}.error / total_error))
        Xlin2 = Xlin2 +  (wi * ensembles{i}.map);
    end
